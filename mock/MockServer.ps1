...
. "$PSScriptRoot\..\Config\MenuConfig.ps1"
...
if ($existingProcess) {
    Write-Host "Port $port is already in use. Attempting to stop the process..." -ForegroundColor Yellow
    Stop-Process -Id $existingProcess.OwningProcess -Force -ErrorAction SilentlyContinue
    Write-Host "Process using port $port has been stopped." -ForegroundColor Green
}

$tcpConnections = Get-NetTCPConnection -LocalPort $port -ErrorAction SilentlyContinue
if ($tcpConnections) {
    Write-Host "Port $port is already in use. Please stop the process using this port and try again." -ForegroundColor Red
    exit
}
