...
function Read-ConfigurationDetails1 {
    [CmdletBinding()]
    param (
    ...
    try {
        Write-Verbose "START $($MyInvocation.MyCommand.Name)"
        Write-Verbose "ConfigFilePath: $SystemConfigFilePath"
...
function Read-MachineConfigurationDetails2 {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
        [string]$MachineName,
    ...
    try {
        ...
        # Extract machine-specific parameters
        $machineNode = $parametersXml.config.parameters.machineConfig.machine | Where-Object {
            $_.machines.machineName -contains $MachineName
        }
        if (-not $machineNode) {
            Throw "Machine configuration for '$MachineName' not found in the parameters file."
        }
...
function Read-MachineConfigurationDetails1 {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
        [string]$MachineName,
    ...
    try {
        ...
        # Extract machine-specific parameters
        $machineNode = $parametersXml.config.parameters.machineConfig.machine | Where-Object { $_.name -eq $MachineName }
        if (-not $machineNode) {
            Throw "Machine configuration for '$MachineName' not found in the parameters file."
        }
...
'@
        ...
        if ($Header) {
            ...
            foreach ($line in $Header) {
                ...
                Write-Host ($centeredLine.PadRight($SeparatorLength)) -NoNewline -ForegroundColor $HeaderColor
        ...
        if ($Title) {
            ...
            Write-Host ("$Vertical") -NoNewline -ForegroundColor $FrameColor
            Write-Host ($centeredTitle.PadRight($SeparatorLength)) -NoNewline -ForegroundColor $TitleColor
            Write-Host ("$Vertical") -ForegroundColor $FrameColor
            Write-Host ($SplitLine) -ForegroundColor $FrameColor
        }
        ...
        for ($i = 0; $i -lt $Options.Length; $i++) {
            ...
            if ($displayName -eq $DividerLine) {
                if ($optionText) {
                    Write-Host ("$Vertical") -NoNewline -ForegroundColor $FrameColor
                    Write-Host ($optionText.PadRight($SeparatorLength)) -NoNewline -ForegroundColor $OptionColor
                    Write-Host ("$Vertical") -ForegroundColor $FrameColor
                    $optionText = ""
                }
            ...
            elseif ($displayName.StartsWith($SubTitlePrefix)) {
                $SubTitleText = "$OptionsSpaces$displayName".Replace($SubTitlePrefix, "") 
                $padding = [math]::Floor(($SeparatorLength - $SubTitleText.Length) / 2)
                $centeredSubtitle = (' ' * $padding) + $SubTitleText                
                Write-Host ("$Vertical") -NoNewline -ForegroundColor $FrameColor
                Write-Host ($centeredSubtitle.PadRight($SeparatorLength)) -NoNewline -ForegroundColor $SubTitleColor
                Write-Host ("$Vertical") -ForegroundColor $FrameColor
                $item--
            }
            else {
                if ($optionText) {
                    $optionText = $optionText + ' ' * ($ColumnSpaces.Length + $OptionsMaxLength - $optionText.Length) + "$OptionsSpaces$('{0:D2}' -f ($item))$OptionsSeparator$displayName"
                    Write-Host ("$Vertical") -NoNewline -ForegroundColor $FrameColor
                    Write-Host ($optionText.PadRight($SeparatorLength)) -NoNewline -ForegroundColor $OptionColor
                    Write-Host ("$Vertical") -ForegroundColor $FrameColor
                    $optionText = ""
        ...
        Write-Host ("$Vertical") -NoNewline -ForegroundColor $FrameColor
        Write-Host ($exitText.PadRight($SeparatorLength)) -NoNewline -ForegroundColor $ExitColor
        Write-Host ("$Vertical") -ForegroundColor $FrameColor        
...
function Invoke-PrettyPrintXml {
    param (
        [Parameter(Mandatory = $true)]
    ...
    try {
        ...
        if ($JsonObject -is [System.Collections.IDictionary]) {
            ...
            foreach ($key in $JsonObject.Keys) {
                Write-Verbose "$($MyInvocation.MyCommand.Name):: Processing key: $key"
        ...
        elseif ($JsonObject -is [System.Collections.IEnumerable] -and -not ($JsonObject -is [string])) {
            # Handle arrays
            Write-Verbose "$($MyInvocation.MyCommand.Name):: Processing array: $JsonObject"
            return $JsonObject | ForEach-Object { Format-JsonObject -JsonObject $_ }
        }
        else {
            # Handle primitive values
            Write-Verbose "$($MyInvocation.MyCommand.Name):: Processing value: $JsonObject"
            return $JsonObject
        }
    }
    catch {
